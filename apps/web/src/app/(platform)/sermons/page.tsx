...
import { BookOpen, Plus, Search, Loader2, Trash2, CheckCircle2, X, Play, Video } from 'lucide-react'
...
export default function SermonsPage() {
  ...
  const handleAddSermon = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Sermon logged into archive!')
      setShowModal(false)
      setTitle('')
      setSpeaker('')
      setSeries('')
      ...
      toast.error('Failed to log sermon.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleDelete = async (id: string) => {
    if (!church?.id || !confirm('Delete sermon archive?')) return
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold tracking-tight text-foreground sm:text-3xl">
            Sermon Vault & AI Repurposing
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Archive sermon recordings, generate AI transcripts, and create social media reels for {church?.name}.
          </p>
        </div>
      ...
      {loading ? (
            ...
            <div key={s.id} className="rounded-2xl border bg-card p-5 shadow-xs flex flex-col justify-between space-y-3">
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-muted-foreground font-semibold">{s.scripture}</span>
                </div>
                <h3 className="font-display text-sm font-bold text-foreground mt-2">{s.title}</h3>
                <p className="text-xs text-muted-foreground mt-1">Preacher: {s.speaker}</p>
              </div>
              ...
              <div className="flex items-center justify-between border-t pt-3">
                {s.mediaUrl ? (
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-xs font-semibold text-purple-500 hover:underline"
                  >
                    <Play className="h-3.5 w-3.5" /> Watch / Listen
                  </a>
                ) : (
                  <span className="text-[10px] text-muted-foreground">No media link</span>
                )}
                <button
                  type="button"
        ...
        <div className="rounded-2xl border bg-card p-12 text-center space-y-3 flex flex-col items-center">
          ...
          <h3 className="font-display text-base font-bold text-foreground">No Sermons Archived</h3>
          <p className="text-xs text-muted-foreground max-w-sm">Log sermon audio or video links to enable AI transcript summaries.</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-2xl border bg-card p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="font-display text-sm font-bold text-foreground">Archive New Sermon</h3>

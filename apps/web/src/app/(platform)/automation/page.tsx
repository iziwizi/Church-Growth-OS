...
import { Zap, Play, Pause, Loader2, Plus, X, CheckCircle2, Trash2 } from 'lucide-react'
import {
  collection,
  query,
  getDocs,
...
export default function AutomationPage() {
  ...
  async function loadWorkflows() {
    ...
    try {
      ...
      if (snap && !snap.empty) {
        ...
        // Seed default workflows on first load
  ...
  const handleAddWorkflow = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setWorkflows((prev) => [
        ...prev,
        { id: ref.id, title: wfTitle.trim(), channel: wfChannel, description: wfDesc.trim(), status: 'active' },
      ])
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-2xl border bg-card p-6 shadow-xl space-y-4 text-xs">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="font-display text-sm font-bold text-foreground">Add Automation Workflow</h3>
